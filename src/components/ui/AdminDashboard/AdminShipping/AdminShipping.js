...
const AdminShipping = () => {
  ...
  //   load data
  useEffect(() => {
    fetch(`http://63.250.41.158:5000/api/v1/shipping/getShippings`)
      .then((res) => res.json())
      .then((data) => {
        if (data?.data.length) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (formData?._id) {
      await UpdateHooks(
        `http://63.250.41.158:5000/api/v1/shipping/updateShippings/${formData?._id}`,
        formData
      );
      ...
      try {
        const response = await axios.post(
          "http://63.250.41.158:5000/api/v1/shipping/addShippings",
          formData
        );

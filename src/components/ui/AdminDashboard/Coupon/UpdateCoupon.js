...
const UpdateCoupon = () => {
  ...
  useEffect(() => {
    fetch(`https://apistore.renixlaboratories.com.bd/api/v1/coupon/getCouponsById/${id}`).then(
      (res) =>
        res.json().then((data) => {
  ...
  const handelUpdate = async (e) => {
    e.preventDefault();
    const BASE_URL = `https://apistore.renixlaboratories.com.bd/api/v1/coupon/updateCoupons/${id}`;
    ...
    <div>
       <div class=" ">
        ...
        <p class="text-lg text-gray-800 mb-8">
          Update essential Coupon for health . Act now to secure the latest
          items
        </p>
      </div>
      <form

...
const AdminOrders = () => {
  ...
  useEffect(() => {
    fetch(`https://serverrenixstore.niroghealthplus.com/api/v1/user/getUsersByNum/${userInfo?._id}`)
      .then((res) => res.json())
      .then((data) => {
        setU(data?.data);
      });
  ...
  useEffect(() => {
    setLoading(true);
    const url = `https://serverrenixstore.niroghealthplus.com/api/v1/order/specific?fieldName=${"status"}&&fieldValue=${nav}&&size=${size}&&page=${page}`;
    try {
      fetch(url)
        .then((res) => res.json())
        .then((data) => {

...
const AdminDashboardSliderImg = () => {
  const [sliders, setSliders] = useState([]);
  const [refresh, setRefresh] = useState(false);
  const [img, setImg] = useState("");
  useEffect(() => {
    fetch(`https://serverrenixstore.niroghealthplus.com/api/v1/slider/getSliders`)
      .then((res) => res.json())
  ...
  const addSliderImage = async () => {
    if (img) {
      await PostHooks(
        "https://serverrenixstore.niroghealthplus.com/api/v1/slider/addSliders",
        { sliderImg: img },
  ...
  const handelDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://serverrenixstore.niroghealthplus.com/api/v1/slider/deleteSliders/${id}`, {
          method: "DELETE",
        }).then((res) => {
          if (res.status === 200) {
            setRefresh(!refresh);
...
export default AdminDashboardSliderImg;

...
const AdminSideBarBanner = () => {
  ...
  useEffect(() => {
    fetch(`https://apistore.renixlaboratories.com.bd/api/v1/sidebarslider/getSliders`)
      .then((res) => res.json())
  ...
  const addSliderImage = async () => {
    if (img) {
      await PostHooks(
        "https://apistore.renixlaboratories.com.bd/api/v1/sidebarslider/addSliders",
        { sliderImg: img },
  ...
  const handelDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://apistore.renixlaboratories.com.bd/api/v1/sidebarslider/deleteSliders/${id}`, {
          method: "DELETE",
        }).then((res) => {
          if (res.status === 200) {
            setRefresh(!refresh);
